...
defmodule KjogviWeb.DumperController do
  use KjogviWeb, :controller

  import Ecto.Query

  def home(conn, _) do
    {:ok, modules} = :application.get_key(:kjogvi, :modules)
  ...
  def resource(conn, %{"id" => id, "resource" => resource}) do
    ...
    conn |> redirect(to: ~p"/dev/dumper/#{resource}/#{id}") |> halt()
  end
  ...
  def resource(conn, %{"resource" => resource} = params) do
    cond do
      ...
      module = to_module(resource, :index) ->
        ...
        render(conn, :index, module: module, records: paginate(repo(resource), query, page))

      # show
      module = to_module(resource, :show) ->
        [id] = Enum.take(resource, -1)
        record = module |> repo(resource).get!(id) |> repo(resource).preload(module.__schema__(:associations))
  ...
  defp paginate(repo, query, %{"page" => page, "page_size" => page_size}) do

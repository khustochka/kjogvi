defmodule KjogviWeb.Live.My.Checklists.IndexTest do
  ...
  test "renders with no checklists", %{conn: conn} do
    {:ok, index_live, _html} = live(conn, ~p"/my/checklists")
  ...
  test "panel shows an unresolved marker for unresolved checklists", %{conn: conn, user: user} do
  ...
  test "panel has no unresolved marker for resolved checklists", %{conn: conn, user: user} do
  ...
  test "panel links to show, edit and counts", %{conn: conn, user: user} do
    ...
    assert has_element?(
             index_live,
             ~s{#checklist-#{checklist.id} a[href="/my/checklists/#{checklist.id}"]}
           )

    assert has_element?(
             index_live,
             ~s{#checklist-#{checklist.id} a[href="/my/checklists/#{checklist.id}/edit"]},
             "Edit"
           )
  ...
  test "pagination with multiple checklists", %{conn: conn, user: user} do
  ...
  describe "search filter" do
    ...
    test "a date filter narrows the listed checklists", %{conn: conn, user: user} do
    ...
    test "the unresolved filter shows only unresolved checklists", %{conn: conn, user: user} do
    ...
    test "submitting the filter patches the URL with the filter params", %{conn: conn, user: user} do
      ...
      assert_patch(index_live, ~p"/my/checklists?date=2024-05-01")
    end
    ...
    test "a filtered URL renders the filtered view directly", %{conn: conn, user: user} do
      ...
      {:ok, index_live, _html} = live(conn, ~p"/my/checklists?date=2024-05-01")
    ...
    test "a taxon_key in the URL restores the autocomplete label", %{conn: conn} do
      ...
      {:ok, index_live, _html} = live(conn, ~p"/my/checklists?taxon_key=#{key}")
    ...
    test "a location_id in the URL narrows to that location", %{conn: conn, user: user} do
      ...
      {:ok, index_live, _html} = live(conn, ~p"/my/checklists?location_id=#{location.id}")

      assert has_element?(index_live, "#checklist-#{match.id}")
      refute has_element?(index_live, "#checklist-#{other.id}")
    end

    test "reset patches back to the bare checklists URL", %{conn: conn, user: user} do
      insert(:checklist, user: user, observ_date: ~D[2024-05-01])
      ...
      assert_patch(index_live, ~p"/my/checklists")
    end
    ...
    test "pagination links carry the active filter", %{conn: conn, user: user} do
      ...
      {:ok, _index_live, html} = live(conn, ~p"/my/checklists?date=2024-05-01")

      assert html =~ "/checklists/page/2?date=2024-05-01"
    end
    ...
    test "shows a no-match message and reset clears the filter", %{conn: conn, user: user} do
      ...
      assert html =~ "No checklists match the current filter."
      refute has_element?(index_live, "#checklist-#{checklist.id}")

defmodule KjogviWeb.Live.Lifelist.IndexTest do
  ...
  test "non-empty year list is indexed by robots", %{conn: conn, user: user} do
    ...
    insert(:observation,
      ...
      checklist: insert(:checklist, user: user, observ_date: ~D[2023-06-07])
    )
  ...
  test "toggling Motorless only in-session empties the list when no motorless checklists exist",
    ...
    checklist = insert(:checklist, user: user, motorless: false)
    insert(:observation, checklist: checklist, taxon_key: Ornitho.Schema.Taxon.key(taxon))
  ...
  test "location checklist shows breadcrumb with ancestors when filtered", %{
    conn: conn,
    user: user
  } do
    ukraine =
  ...
  test "location checklist shows World as bold when no location filter", %{conn: conn, user: user} do
  ...
  test "year header is omitted when filtered by year", %{conn: conn, user: user} do
    {taxon, _} = Factory.create_species_taxon_with_page()
    checklist = insert(:checklist, user: user, observ_date: ~D"2023-05-01")
    insert(:observation, checklist: checklist, taxon_key: Ornitho.Schema.Taxon.key(taxon))
  ...
  describe "community lifelist" do
    ...
    test "filter links point back to the community URL space", %{conn: conn, user: user} do
      insert(:observation,
        ...
        checklist: insert(:checklist, user: user, observ_date: ~D"2023-07-16")
      )

defmodule KjogviWeb.UserSessionControllerTest do
  ...
  describe "POST /account/login" do
    test "logs the user in", %{conn: conn, user: user} do
      ...
      assert response =~ ~p"/my/settings/profile"
      assert response =~ ~p"/account/logout"
    end
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_kjogvi_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
    ...
    test "login following password update", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/my/settings/security"
  ...
  describe "DELETE /account/logout" do
    test "logs the user out", %{conn: conn, user: user} do
      conn = conn |> login_user(user) |> delete(~p"/account/logout")
      assert redirected_to(conn) == ~p"/"

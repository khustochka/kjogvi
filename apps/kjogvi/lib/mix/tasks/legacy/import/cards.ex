defmodule Mix.Tasks.Legacy.Import.Cards do
  ...
  def run(_args) do
    ...
    for i <- 0..div(cards_num - 1, 1000) do
      results = Postgrex.query!(pid, "SELECT * FROM cards ORDER BY id LIMIT 1000 OFFSET #{1000 * i}", [])

      columns = results.columns |> Enum.map(&String.to_atom/1)

      cards = for row <- results.rows do
  ...
  defp transform_keys(%{locus_id: loc_id, autogenerated: autogenerated, created_at: created_at, updated_at: updated_at, start_time: start_time} = card) do

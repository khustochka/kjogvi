defmodule Kjogvi.Legacy.Import.Locations do
  ...
  # Legacy import is out of scope for the location hierarchy redesign. The old
  # importer wrote the now-dropped `ancestry` / `cached_public_location_id`
  # columns (and never populated the new level FKs), so `import/3` and its
  # helpers are disabled — left commented below for reference until (if ever)
  # the importer is rebuilt on the level-FK model. See
  # docs/location-hierarchy-redesign.md ("ignore legacy import").
  ...
  def import(_columns_str, _rows, _opts) do
